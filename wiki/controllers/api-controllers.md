...
TLDR: REST API for agent/programmatic access. REST v1 accepts project API keys and Doorkeeper OAuth bearer tokens; the legacy MCP binary-upload endpoint accepts a separate single-use bearer credential.
...
**Inherits from:** `ActionController::API` (uses a purpose-specific upload bearer rather than persistent API/OAuth auth)
...
- Requires the one-time upload token generated by the target [[models/screenshot-image]] as `Authorization: Bearer <token>`; URL/query credentials are rejected
- Resolves the credential before comparing the parent ID, so an invalid bearer cannot enumerate screenshot existence
