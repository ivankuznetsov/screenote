...
TLDR: A comment pinned to a point or rectangular region of a screenshot. Uses percentage-based coordinates for resolution independence. Has open/resolved status with resolve/reopen operations that create audit trail via [[annotation-comment]].
...
- `as_api_json` -- Serializes for API/MCP response (id, type, coordinates, comment, status, author email, comments_count)
...
See also: [[screenshot]], [[annotation-comment]], [[services/annotation-crop-service]]
