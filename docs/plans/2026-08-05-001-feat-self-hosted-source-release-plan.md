...
- Project API keys remain project-scoped actors, but each key records the active owner who issued it. Suspending that issuer revokes the key in the same credential transaction; keys issued by another active owner remain valid. Existing keys backfill to the current project creator because the existing UI permits only that owner to create them.
