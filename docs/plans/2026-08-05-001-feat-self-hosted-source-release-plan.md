...
The changed-security coverage gate starts instrumentation before Rails boot and uses an explicit positive source manifest for the seven named security domains. It resolves the branch's `origin/main` merge base, rejects missing or uninstrumented manifest files and invalid or empty diffs, and requires 100% of executable changed lines and changed branch arms. It does not lower the legacy whole-application threshold or infer safety from unrelated legacy files.
