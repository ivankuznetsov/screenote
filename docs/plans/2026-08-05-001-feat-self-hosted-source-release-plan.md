...
- R27. Before upgrading, an operator must create one consistent, restorable backup set containing the persistent volume, either the local blobs within it or a corresponding snapshot/copy of S3-compatible objects, and the archived application configuration and non-derivable application secrets required to restore the instance. The operator must separately escrow the age identity and independent random authenticity key; neither recovery factor may be inside the backup set or application data. Rollback means stopping the new image, authenticating and restoring that exact pre-upgrade backup set, and starting the predecessor's immutable image—never running old code against migrated state—and may lose writes made after the backup boundary.
...
- F4. **Upgrade or roll back an instance**
  ...
  - **Actors:** A1.
  - **Steps:** A1 confirms the named predecessor/successor pair, verifies separately escrowed age and authenticity recovery factors, stops writes as documented, captures one consistent backup set covering the volume, selected blob store, and archived application secrets/configuration, then starts the successor's immutable image and verifies health and data. If verification fails, A1 stops it, authenticates and restores that exact set with both external recovery factors, and starts the predecessor's immutable image; writes after the backup boundary are discarded.
...
|---|---|---|
| Ruby and Bundler | Ruby 3.4.10; Bundler 2.7.2 | Build and runtime evidence must name the resolved versions |
...
| Server and image | Puma 8.0.2; Thruster 0.1.23; digest-pinned Ruby 3.4.10 Alpine 3.24 base | Pin or attest mutable build inputs and verify graceful one-container supervision |
...
- KTD6. **Make invitations the only post-claim admission path.** Route browser and agent invitation issuance through one owner-authorized domain service that enforces SaaS quotas only in SaaS mode, records the issuing owner, returns the same private acceptance URL to an authorized owner, and enqueues mail only after commit when mail is configured. Normalize email before a database-enforced pending-invitation uniqueness check. All authority-changing operations use one global order: `Installation` when applicable, a normalized-email admission lock, every involved `User` in ascending ID order, every involved `Project` in ascending ID order, `ProjectInvitation` rows in ascending ID order, memberships, then authentication/other credentials. PostgreSQL uses a transaction-scoped advisory lock over a stable domain-separated SHA-256-derived 64-bit normalized-email key; production SQLite begins the outer write transaction in immediate mode. Acceptance locks and revalidates the invitation, its unexpired state, the issuer's active owner authority, and the authenticated identity or provider-verified email match; it then atomically creates or binds the user and membership and creates a browser session only after commit. Two different-project acceptances for the same unseen email may create one user; the loser continues only after its local password or exact provider identity authenticates that committed user, never from matching email alone. Suspension or ownership loss cancels that issuer's pending invitations in the same authority-changing transaction, including races with acceptance. A new local user sets a durable password during acceptance; a social provider may create an identity only while consuming a matching invitation. Ordinary signup, OAuth auto-creation, test-token user creation, production seeds, magic-link signup, and every other account-creation seam remain closed. Only outermost services retry deadlock, serialization, lock-timeout, or SQLite busy/locked failures, at most three times; exhaustion returns a stable retryable result without losing token context. Covers R10-R12, R14, R24.
...
- KTD17. **Make bearer secrets digest-only, purpose-bound, and non-loggable.** Enable Doorkeeper hashing for access tokens, refresh tokens, and retained confidential client secrets with an upgrade that preserves existing clients. Because existing rolling SaaS containers cannot read transformed values, this first security migration uses a declared maintenance cutover: stop and drain every old web/worker process, take the KTD11 backup, migrate once, start only the new revision, and rollback only by restoring the full backup before starting the predecessor. Store invitation, password-reset, magic-link, and recovery credentials as digest-only `AuthenticationToken` records bound to exact purpose/subject, monotonically increasing generation, random derivation ID, derivation-key ID, expiry, and one atomic terminal state. Derive 32 secret bytes from a versioned, domain-separated HMAC key and canonical tuple; store only SHA-256 of those bytes. A URL renders the bytes as base64url in a fragment and the no-JavaScript fallback renders the same bytes as grouped Base32, so both exchange into one digest. Derive the primary key from `SECRET_KEY_BASE`; routine rotation retains explicitly file-backed prior keys until no active token references them, missing required keys fail preflight, and emergency rotation supersedes/reissues outstanding tokens before removing the compromised key. Validation must resolve the stored key ID and rederive as well as compare the stored digest, so removing a key cannot leave old raw credentials usable. Mail jobs carry only subject/token IDs and rederive after reload. Use automatic `GET /authentication-links/:purpose` fragment-to-`POST /authentication-links/:purpose/exchange` as the primary interaction and a labelled manual-code fallback; successful exchange stores only token ID and purpose in the encrypted session and redirects to a tokenless domain route. Both paths share exchanging, invalid, expired, already-used, retryable-failure, and success states. Raw values never enter request paths, query strings, referrers, caches, proxy/app logs, Honeybadger, model attributes, job arguments, or session state; GET never consumes a credential or creates a session. Legacy upload credentials move to authorization headers. Covers R10-R12, R19-R21, R25, R28, R29.
...
| Backup recovery material is lost | A valid backup becomes intentionally unrecoverable | Require an operator-held age identity plus an independent random authentication key outside the host/backup, verify both for every operation, and document separate escrow/rotation without copying either into the set |
...
- **Dependencies:** U1-U3.
- **Files:** `db/migrate/*_harden_user_and_invitation_identity.rb`, `db/migrate/*_create_authentication_tokens.rb`, `app/models/installation.rb`, `app/models/installation_audit_event.rb`, `app/models/project_invitation.rb`, `app/models/authentication_token.rb`, `app/services/admission_lock.rb`, `app/services/database_retry.rb`, `app/services/authentication_links/keyring.rb`, `app/services/authentication_links/deriver.rb`, `app/services/authentication_links/presentation.rb`, `app/services/installations/claim.rb`, `app/services/project_invitations/issue.rb`, `app/services/project_invitations/accept.rb`, `app/services/project_invitations/cancel.rb`, `app/services/project_invitations/cancel_for_issuer.rb`, `app/controllers/authentication_links_controller.rb`, `app/controllers/bootstrap_controller.rb`, `app/controllers/invitation_acceptances_controller.rb`, `app/controllers/project_invitations_controller.rb`, `app/controllers/omniauth_callbacks_controller.rb`, `app/controllers/application_controller.rb`, `app/controllers/oauth/test_tokens_controller.rb`, `app/controllers/registrations_controller.rb`, `app/controllers/sessions_controller.rb`, `app/controllers/passwords_controller.rb`, `app/controllers/confirmations_controller.rb`, `config/routes.rb`, `db/seeds.rb`, `app/javascript/controllers/bearer_token_exchange_controller.js`, `app/views/layouts/authentication_link.html.erb`, `app/views/authentication_links/show.html.erb`, `app/views/bootstrap/show.html.erb`, `app/views/invitation_acceptances/show.html.erb`, `app/views/project_memberships/index.html.erb`, `app/views/sessions/new.html.erb`, `test/migrations/identity_upgrade_test.rb`, `test/models/installation_test.rb`, `test/models/project_invitation_test.rb`, `test/models/authentication_token_test.rb`, `test/services/installations/claim_test.rb`, `test/services/project_invitations/accept_test.rb`, `test/controllers/bootstrap_controller_test.rb`, `test/controllers/invitation_acceptances_controller_test.rb`, `test/integration/bootstrap_concurrency_test.rb`, `test/integration/invitation_acceptance_concurrency_test.rb`, `test/system/self_hosted_admission_test.rb`.
- **Approach:** Preflight existing identity collisions before canonicalization, then add KTD15 constraints without silently merging records. Pull the minimal `users.access_status` and `InstallationAuditEvent` foundations forward from U5 because claim and issuer revalidation require them. Put each claim or acceptance transition inside one primary transaction that starts with the production SQLite/PostgreSQL locking strategy and follows KTD6's global order; extend membership removal to lock actor and target users in ascending ID order before its project. Return deterministic semantic results for uniqueness races and retryable busy failures, revalidate/cancel issuer authority, and schedule only ID-based after-commit side effects. Draw app-owned auth routes/controllers so ordinary registration and provider auto-creation cannot bypass these services. Use KTD17's sterile local-only authentication-link layout, automatic fragment exchange, manual-code fallback, and one shared state model; attach no third-party resource and make every token response non-cacheable/non-referring/noindex. Replace legacy signed invitation/reset/magic/confirmation URLs at cutover rather than dual-reading raw path tokens; preserve invitation rows, supersede outstanding credentials, and require owners to recopy/reissue. Specify the invitation interaction tree: pending rows expose Copy link with private-link warning/confirmation and Cancel; entry shows project, inviter, and invited address; matching sessions accept directly; existing users resume after sign-in; new users choose local password or an enabled verified-email provider; mismatched sessions can switch account; terminal links direct the invitee to the owner; success enters the named project. Preserve SaaS's account model behind KTD1 rather than applying self-hosted bootstrap globally.
- **Frozen service results:** `Installations::Claim.call` returns `claimed`, `already_claimed`, `invalid_token`, `email_taken`, `invalid`, `unavailable`, or `retryable_busy`. `ProjectInvitations::Issue.call` returns `issued`, `already_pending`, `reissued`, `forbidden`, `inactive_issuer`, `already_member`, `limit_reached`, `invalid`, `not_found`, or `retryable_busy`. `ProjectInvitations::Accept.call` accepts one immutable identity proof (current session, local password, or exact provider UID plus provider-verified normalized email) and returns `accepted`, `authentication_required`, `identity_mismatch`, `invalid_identity`, `invalid`, `expired`, `cancelled`, `already_used`, `superseded`, `issuer_revoked`, `limit_reached`, or `retryable_busy`. `ProjectInvitations::Cancel.call` returns `cancelled`, `already_cancelled`, `already_accepted`, `forbidden`, `not_found`, or `retryable_busy` and never destroys the row. `CancelForIssuer.call` requires its caller's open transaction and locked issuer, performs no retry/commit, and returns `cancelled` or `none`.
- **Test scenarios:**
...
- **Files:** `app/controllers/health_controller.rb`, `config/routes.rb`, `compose.yaml`, `bin/docker-entrypoint`, `bin/self-host-diagnostics`, `bin/self-host-backup`, `bin/self-host-restore`, `bin/saas-credential-cutover`, `.kamal/hooks/post-deploy`, `lib/screenote/self_hosted/backup_set.rb`, `lib/screenote/self_hosted/host_operations.rb`, `script/self_hosted_backup_internal`, `script/self_hosted_restore_internal`, `script/self_hosted_backup_smoke`, `script/saas_credential_cutover_smoke`, `docs/self-hosting/backup-and-restore.md`, `docs/self-hosting/upgrades.md`, `test/controllers/health_controller_test.rb`, `test/integration/self_hosted_readiness_test.rb`, `test/integration/self_hosted_s3_test.rb`, `test/integration/self_hosted_backup_restore_test.rb`, `test/integration/self_hosted_operations_command_test.rb`.
- **Approach:** Keep public readiness generic/local and expose redacted detail through the selected-provider diagnostics command. Implement `bin/self-host-backup` and `bin/self-host-restore` as the only supported production interfaces, with stable exit codes and explicit external archive destination, age recipient/identity, independent operator-held authentication-key file, configuration/secret bundle, S3 snapshot/copy evidence, and empty-volume target arguments; the smoke script calls only those commands. Backup proves the running image, secret mounts, and named data volume equal the sanitized rendered Compose model, enters maintenance, rejects new uploads, drains requests/jobs, and aborts rather than taking a partial set. Encrypt the complete volume to the age recipient and origin-authenticate the encrypted manifest digest with the independent random key, pair S3 mode with one encrypted provider snapshot/copy, and finalize only after the manifest binds checksums, exact restore-image digest, conditional predecessor declaration, all four schema versions, the non-secret configuration fingerprint, secret-bundle reference, and every database-referenced object. Restore authenticates before Docker mutation and manifest decryption, writes no persistent copy of the age identity, restores only into an empty volume, validates manifest/object inventory/database constraints, reclaims stale queue work, and starts the recorded immutable restore image. Establish an initial-release fixture and require each future release PR to add its immediate predecessor to the upgrade matrix. Replace the current rolling Kamal `post-deploy` migration with an explicit KTD17 cutover: a normal deploy must refuse while the credential-hardening migration is pending; the cutover command must first quiesce and prove every predecessor web and worker process stopped, then execute a pre-reviewed digest-pinned backup hook and verify its new evidence is bound to that exact stopped window and database restore point, run the migration once from the immutable successor image, verify raw legacy access and refresh lookups through the new runtime, and start only that revision. No hook may transform credentials after mixed revisions are serving.
- **Test scenarios:**
  ...
  - S3-mode backup pairs the same quiesced volume state with the matching encrypted object snapshot and rejects plaintext, tampered, unfinalized, absent/version-mismatched object, wrong configuration fingerprint, altered archive, missing secret bundle, wrong schema, or wrong image. A public-recipient forgery, wrong authenticity key, missing age identity, key alias into archived input, or mismatched running storage volume fails clearly before destructive restore work or service quiescence where applicable.
...
The changed-security coverage gate uses an explicit positive source manifest for the seven named security domains. It validates the exact event comparison commit, the manifest discovery union, and guarded-path membership at both the base and current revision before deciding applicability; removing a previously guarded path fails closed. Missing, malformed, unavailable, non-ancestor, or genuinely empty overall comparisons fail closed; a non-empty comparison with no manifest-path changes reports an explicit not-applicable success without starting coverage instrumentation. When a manifest path changed, instrumentation starts before Rails boot and requires 100% of executable changed lines and changed branch arms. It does not lower the legacy whole-application threshold or infer safety from unrelated legacy files.
...
| Coverage for changed security paths | `script/release_test_matrix coverage` / required `CI / coverage` | Before PR handoff and every PR | Merged SaaS/self-hosted runs cover every new deployment, bootstrap, invitation, principal, suspension, recovery, and transfer line/branch without excluding feature files |
...
- Backups are age-encrypted with an operator-held identity and origin-authenticated with an independent random operator key, both outside the host and set; public release evidence is redacted, detailed security evidence is access-controlled, and both exact platform images pass secret and vulnerability gates.
