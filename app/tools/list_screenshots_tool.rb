...
class ListScreenshotsTool < ApplicationTool
  ...
  description "List screenshots in a project, with annotation counts. Supports pagination via limit/offset."
  ...
  def call(project_id:, status: nil, limit: 50, offset: 0)
    ...
    with_error_handling do
...

      {
        screenshots: screenshots.map do |s|
          url = Rails.application.routes.url_helpers.project_screenshot_url(project, s)
